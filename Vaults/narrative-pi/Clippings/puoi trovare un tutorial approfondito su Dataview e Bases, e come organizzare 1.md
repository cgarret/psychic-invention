...
Obsidian lavora al meglio con file **Markdown** salvati nella cartella del vault.obsidian+1
...
- Salva/esporta il testo in un formato semplice:
	- o copia‑incolla in un nuovo file `.md` dentro la cartella `Capitoli` del vault,
		- oppure, se il tuo Word lo permette, esporta in `.txt` /`.md` e poi sposta il file in Obsidian.[obsidian](https://obsidian.md/it/help/import/markdown)
- Ripeti per ogni capitolo (anche solo per i 3–4 più recenti per iniziare).

Obsidian ha strumenti di import per markdown e formati semplici, quindi una volta che i testi sono in `.md` e nella cartella del vault, per lui sono note normali.obsidian+1
...
Quando i capitoli sono in Obsidian, il passo chiave è dare a ciascuno le **properties** standard che userai in Dataview/Bases.kevsrobots+1
...
Dopo aver fatto questo per 3–4 capitoli, vale la pena creare un **template “Capitolo Romanzo”** con queste properties già pronte, per i nuovi capitoli che scriverai direttamente in Obsidian.obsidian+1
...
- Crea una **Base** sulla cartella `Capitoli` del romanzo.
	- Obsidian (con Bases abilitato) ti permette di scegliere la cartella come sorgente e vedere tutte le note capitolo in tabella.obsidian+1
		- Aggiungi colonne per `numero_capitolo`, `stato_bozza`, `parole_target`, `parole_reali`.[youtube](https://www.youtube.com/watch?v=34NAe62IH-M) [kevsrobots](https://www.kevsrobots.com/learn/obsidian/08_dataview_and_bases.html)
		- Se vedi i capitoli corretti e riesci a cambiare `stato_bozza` dalla tabella, la migrazione è “viva”.
- Crea nella nota `MOC – Romanzo` una prima **query Dataview** molto semplice che elenchi tutti i capitoli con `tipo = "capitolo"` ordinati per `numero_capitolo`.[blacksmithgu.github](https://blacksmithgu.github.io/obsidian-dataview/) [youtube](https://www.youtube.com/watch?v=buOxN65U0qE)
...
3. Copia‑incolla il testo dal Word alla nota.  
	Obsidian lavora nativamente con file Markdown, quindi il semplice copia‑incolla in una nota va benissimo per iniziare.obsidian+1

Non curarti troppo della formattazione: l’importante ora è **avere 3 note capitolo separate** dentro la cartella `Capitoli`.[obsidian](https://obsidian.md/it/help/import/markdown)
...
Questo è il pezzo che permette poi a Bases/Dataview di funzionare bene.kevsrobots+1

Se vedi che questo schema ti torna, potrai trasformarlo in **template “Capitolo Romanzo”** e non pensarci più per i futuri capitoli.[obsidian](https://obsidian.md/it/help/plugins/file-explorer)
...
- Crea una **Base** nuova e scegli come sorgente la cartella `Capitoli`.obsidian+1
- Aggiungi colonne:
	- `numero_capitolo`
		- `stato_bozza`
		- `parole_target`
- Ordina per `numero_capitolo`.[youtube](https://www.youtube.com/watch?v=34NAe62IH-M)

Se vedi comparire i 3 capitoli in tabella e riesci a cambiare `stato_bozza` da lì, vuol dire che il modello funziona e vale la pena migrare gli altri 7 allo stesso modo.youtube+1
...
- una prima query Dataview che elenca i capitoli in ordine di `numero_capitolo`.[blacksmithgu.github](https://blacksmithgu.github.io/obsidian-dataview/) [youtube](https://www.youtube.com/watch?v=buOxN65U0qE)
...
- aprire se vuoi la Base dei capitoli per modificare lo stato in modo “da foglio di calcolo”.youtube+1
...
Questi sono quelli che serviranno subito a Bases e Dataview e nascono da tutto il lavoro che hai già fatto su POV, archi, trigger, stati interni.[02\_CH\_TheIntrusion\_r00.md](https://ppl-ai-file-upload.s3.amazonaws.com/web/direct-files/attachments